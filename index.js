...
const devs = ["769590521328828457","707931169597030471","738551496236597378","850022889369632818"]
setTimeout(() => {
...
client.on("message",async message =>{
  ...
  .setColor("BLUE").setAuthor(message.author.username , message.author.displayAvatarURL({dynamic:true})).setTimestamp().setFooter("highWorld", message.guild.iconURL()).setTitle("**High World luck spins**").addField("Spin Normel" , "تحتاج الى invites واحد للحصول على نقطه.").addField("Legend spin","تحتاج إلى invites اثنين للحصول على نقطتين.")
...
collector.on('collect',b =>{
  ...
  let praise = rewn[Math.floor(Math.random() * rewn.length)];
     message.lineReplyNoMention(`Congratulat you won ${praise}`)
...
client.on("message" , message =>{
  ...
  if(message.content == '$off spins'){
    ...
    mdb.onoff = "on";
		message.channel.send("Spins is on now :)");
	}
	fs.writeFile('./onoff.json', JSON.stringify(mdb), function(e) {
		if (e) throw e;
	})
...
client.on("ready",()=>{
client.user.setActivity("high World Luck.", { type: 'WATCHING' })
